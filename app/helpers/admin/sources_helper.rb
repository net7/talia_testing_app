module Admin::SourcesHelper
  ...
  def data_records_column(record)
    data = record.data_records.collect { |dr| dr.location }
    data.join(', ')
  end

  def data_records_form_column(record, input_name)
    "#{h(input_name)} - #{h(record.inspect)}"
  end
