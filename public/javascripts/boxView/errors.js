...
function checkJson(json, ret) {
    ...
    try {
        ...
        //
        // this works only in some browsers (firefox)
        // we could render a json object if this worked
            var h = eval ("("+JSON.stringify(json)+")");

        // we needed to revert to a string json. (it seems that spaces before/after
        // parenteses somewhat help browsers...
//        var h = eval (" ("+json+") ");
...
$.ajaxSetup({
    ...
    error: function(err) {
        displayError("AJAX network error.<br/><br/>Try again later...");
    },
    complete: function() {
        hideLoading();
    }
